...
function renderCombatVitalsV149(r){const el=$('combatVitalsV149');if(!el)return;if(!profile){el.style.display='none';return}el.style.display='grid';const run=r?.run,c=CLASSES[profile.classId],active=run?.players?.[peerId],base=active||trainingPlayer(),hp=active?Math.max(0,active.hp):base.maxHp,maxHp=Math.max(1,base.maxHp||1),res=active?Math.max(0,active.res):base.maxRes,maxRes=Math.max(1,base.maxRes||c.maxRes||1),state=active?(active.dead?'FALLEN':active.downed?'DOWNED':run.bossTelegraph?'TELEGRAPH':run.phase==='player'?'HUNTING':run.phase.toUpperCase()):worldStateV14()?'SURFACE':'EMBERWATCH';$('combatVitalsNameV149').textContent=`${profile.name} • ${c.name}`;$('combatVitalsStateV149').textContent=state;$('combatVitalsGearV149').textContent=`GR ${gearRating(profile)}`;$('combatResLabelV149').textContent=c.res.toUpperCase().slice(0,5);$('combatHpFillV149').style.width=`${clamp(hp/maxHp*100,0,100)}%`;$('combatResFillV149').style.width=`${clamp(res/maxRes*100,0,100)}%`;$('combatHpTextV149').textContent=active?`${hp}/${maxHp}`:'READY';$('combatResTextV149').textContent=active?`${res}/${maxRes}`:'READY'}
...
function renderAll(){
  const r=room||snapshot;updateHudModeV141(r);draw(r?.run);renderParty(r);renderPlayer(r);renderRunLoot(r);renderLog(r);renderActions(r);renderCombatMeters(r);renderMinimap(r);renderGrandHuntHud(r?.run);renderCombatHudV11(r);renderCombatVitalsV149(r);renderWorldClockV149();syncHudSignalsV13(r);const wb=$('worldRegionBannerV14');if(wb)wb.classList.toggle('show',!!worldStateV14()&&!r?.run);renderLobby()
}
...
function renderActions(r){
  ...
  const targetInfo=$('targetInfo'),targetDistance=target&&me?distanceToEnemy(me,target):null;targetInfo.dataset.commandState=commandState;
  if(target){
    ...
    targetInfo.innerHTML=`<div class="target-ribbon-v148"><span class="target-lock-v148 ${target.boss?'boss':alerted?'alert':''}">${target.boss?'BOSS':alerted?'ALERT':'LOCKED'}</span><b class="target-name-v148 ${target.boss?'gold':''}">${target.name}</b>${targetDistance!=null?`<span class="target-range-v148">${targetDistance} TILE${targetDistance===1?'':'S'}</span>`:''}<span class="target-hp-meter-v148"><i style="width:${hpPct}%"></i></span><span class="target-hp-v148">${Math.max(0,target.hp)}/${target.maxHp}</span>${intentLabel?`<span class="target-intent-v148">⚠ ${intentLabel}</span>`:''}<span class="target-more-v148">DETAILS</span></div><div class="target-detail-card-v148"><div><b>${target.boss?'BOSS KIT':'ABILITY'}:</b> ${enemyAbilityTextV09(target)}</div><div class="target-detail-stats-v148">DEF ${target.def}${target.boss?` • COLOSSAL ${target.size}×${target.size} • PHASE ${target.bossPhase||1} • ${target.partySize}P SCALE`:''}${target.trait?` • ${target.trait}`:''}${!target.boss?` • ${alerted?'ALERTED':`DORMANT • AGGRO ${enemyAggroRangeV132(target)}`}`:''}</div>${target.boss&&bossCounterHintV131(target)?`<div class="counter-hint-v131"><b>COUNTERPLAY:</b> ${bossCounterHintV131(target)}</div>`:''}${target.intent?`<div class="bosswarn">⚠ <span class="boss-ability">${intentLabel}</span> CHARGING</div>`:''}${run.stageEvent?`<div class="eventwarn">${run.stageEvent.name}: ${run.stageEvent.desc}</div>`:''}<div class="target-shortcuts-v131"><kbd>Tab</kbd> cycle enemy • <kbd>Q</kbd> cycle body part</div></div>`;
  }else{
...
function spriteAnimationLoop(ts){
  ...
  if(ts-lastMeterPaint>=180){renderCombatMeters(room||snapshot);renderMinimap(room||snapshot);renderWorldClockV149();renderCombatVitalsV149(room||snapshot);lastMeterPaint=ts}
