...
function emptyRemoteTransportV147(){return{room:null,roomSessionId:null,memberToken:null,inviteToken:null,authorityEpoch:1,hostPeerId:null,cursor:0,knownSnapshotVersion:0,clientSeq:0,controller:null,ready:Promise.resolve(false),postQueue:Promise.resolve(),connected:false,authorized:false,localFallback:false,idlePolls:0,nextPollDelay:500,heartbeatAt:0,claiming:false,snapshotTimer:null,pendingSnapshot:null,departing:false,recoveryRequired:false,recoveryNoticeShown:false}}
...
function stopRemoteTransportV141(){const old=remoteTransportV141;old.controller?.abort();if(old.snapshotTimer)clearTimeout(old.snapshotTimer);if(bc)bc.close();bc=null;remoteTransportV141=emptyRemoteTransportV147();document.body.classList.remove('remote-recovery-required-v147');setRemoteTransportStatusV141(false)}
...
function setRemoteRecoveryRequiredV147(state,required){if(remoteTransportV141!==state)return;const next=!!required,changed=state.recoveryRequired!==next;state.recoveryRequired=next;document.body.classList.toggle('remote-recovery-required-v147',next);if(next&&!state.recoveryNoticeShown){state.recoveryNoticeShown=true;notifyV11('warn','HOST RECONNECT REQUIRED','The active field is frozen until the same host returns.',{life:7000})}else if(!next)state.recoveryNoticeShown=false;if(changed)renderAll()}
function observeAuthorityV147(authority,state=remoteTransportV141){if(!authority||remoteTransportV141!==state||authority.roomSessionId!==state.roomSessionId||authority.protocolVersion!==NETWORK_PROTOCOL_V147)return;if(authority.authorityEpoch<state.authorityEpoch)return;const epochChanged=authority.authorityEpoch!==state.authorityEpoch;state.authorityEpoch=authority.authorityEpoch;state.hostPeerId=authority.hostPeerId;const selfHost=authority.hostPeerId===peerId;if(selfHost)isHost=true;else if(isHost)isHost=false;setRemoteRecoveryRequiredV147(state,authority.recoveryRequired===true&&!selfHost);applyAuthoritySnapshotV147(authority,state);saveRemoteSessionV147(state);if(epochChanged&&selfHost&&!room?.run&&!room?.worldV14){if(room){room.hostPeerId=peerId;room.authorityEpochV147=state.authorityEpoch}notifyV11('info','PARTY LEADERSHIP RECOVERED','Authority transferred safely at Emberwatch.');setTimeout(broadcastSnapshot,0)}if(authority.leaseExpired&&authority.candidatePeerId===peerId&&!state.recoveryRequired)claimCampAuthorityV147(state)}
...
const V149_GEAR_VISUALS=Object.freeze({head:'head',shoulders:'shoulders',chest:'chest',gloves:'gloves',boots:'boots',weapon:'weapon',offhand:'offhand',ring:'ring',necklace:'necklace'});
function gearVisualTypeV149(value){const type=canonicalItemTypeV132(typeof value==='string'?value:value?.type);return V149_GEAR_VISUALS[type]||'chest'}
function gearSpriteHTMLV149(value,opts={}){const item=typeof value==='object'&&value?value:null,type=gearVisualTypeV149(value),rarity=String(opts.rarity||item?.rarity||'common').toLowerCase(),enhance=Math.max(0,Math.min(5,Number(opts.enhance??item?.enhance)||0)),label=opts.label||`${slotDisplayTypeV132({type})} equipment`;return `<span class="gear-object-v149 gear-${type} rarity-${rarity}" role="img" aria-label="${label}"><i></i>${enhance?`<em>+${enhance}</em>`:''}</span>`}
function drawGearObjectV149(value,x,y,rarity='uncommon',scale=1){const type=gearVisualTypeV149(value),colors={common:'#9aa8b8',uncommon:'#72cc8b',rare:'#65a8ff',epic:'#bd7dff',legendary:'#f1a052',relic:'#f5dc82',mythic:'#ff629b'},c=colors[rarity]||colors.uncommon,s=Math.max(1,scale);ctx.save();ctx.translate(x,y);ctx.strokeStyle=c;ctx.fillStyle=c;ctx.lineWidth=2*s;ctx.shadowColor=c;ctx.shadowBlur=5*s;const line=(x1,y1,x2,y2)=>{ctx.beginPath();ctx.moveTo(x1*s,y1*s);ctx.lineTo(x2*s,y2*s);ctx.stroke()};if(type==='weapon'){ctx.rotate(.68);ctx.fillRect(-2*s,-11*s,4*s,20*s);ctx.fillRect(-7*s,5*s,14*s,3*s);ctx.beginPath();ctx.moveTo(-3*s,-11*s);ctx.lineTo(0,-16*s);ctx.lineTo(3*s,-11*s);ctx.fill()}else if(type==='ring'){ctx.beginPath();ctx.arc(0,0,7*s,0,Math.PI*2);ctx.stroke()}else if(type==='necklace'){ctx.beginPath();ctx.arc(0,-2*s,9*s,.2,Math.PI-.2);ctx.stroke();ctx.fillRect(-2*s,6*s,4*s,4*s)}else if(type==='offhand'){ctx.beginPath();ctx.moveTo(0,-11*s);ctx.lineTo(9*s,-6*s);ctx.lineTo(7*s,6*s);ctx.lineTo(0,12*s);ctx.lineTo(-7*s,6*s);ctx.lineTo(-9*s,-6*s);ctx.closePath();ctx.stroke();line(0,-8,0,8)}else if(type==='head'){ctx.strokeRect(-8*s,-7*s,16*s,14*s);line(-6,0,6,0)}else if(type==='chest'){ctx.beginPath();ctx.moveTo(-8*s,-9*s);ctx.lineTo(8*s,-9*s);ctx.lineTo(10*s,-4*s);ctx.lineTo(7*s,11*s);ctx.lineTo(-7*s,11*s);ctx.lineTo(-10*s,-4*s);ctx.closePath();ctx.stroke();line(0,-7,0,8)}else if(type==='shoulders'){ctx.strokeRect(-11*s,-3*s,8*s,8*s);ctx.strokeRect(3*s,-3*s,8*s,8*s);line(-3,0,3,0)}else if(type==='gloves'){ctx.strokeRect(-7*s,-4*s,14*s,11*s);for(let i=-5;i<=5;i+=3)line(i,-4,i,-10)}else{line(-7,-9,-7,7);line(-7,7,0,7);line(4,-9,4,7);line(4,7,11,7)}ctx.restore()}
...
function ensureWorldProfileV14(p){if(!p)return;p.worldV14=p.worldV14||{discoveredRegions:['emberwood'],contracts:{wolves:0,resources:0,elites:0},contractsClaimed:{},waypoints:['emberwatch'],enteredLowlands:false,surfaceClears:0};if(p.worldV14.enteredLowlands==null)p.worldV14.enteredLowlands=false;if(p.worldV14.surfaceClears==null)p.worldV14.surfaceClears=0;if(!Number.isFinite(p.worldV14.clockMinutesV149))p.worldV14.clockMinutesV149=420;if(!Number.isFinite(p.worldV14.worldDayV149))p.worldV14.worldDayV149=1;return p.worldV14}
const V149_WORLD_MINUTES_PER_REAL_MINUTE=60;
function worldClockStateV149(w=worldStateV14(),now=Date.now()){
  const saved=profile?ensureWorldProfileV14(profile):null,clock=w?.clockV149||{baseMinute:saved?.clockMinutesV149??420,day:saved?.worldDayV149??1,epochMs:now,minutesPerRealMinute:0},elapsed=Math.max(0,now-(clock.epochMs||now)),advanced=elapsed/60000*(clock.minutesPerRealMinute??V149_WORLD_MINUTES_PER_REAL_MINUTE),total=Math.max(0,(clock.day-1)*1440+(clock.baseMinute||0)+advanced),minuteOfDay=Math.floor(total%1440),day=Math.floor(total/1440)+1,hour=Math.floor(minuteOfDay/60),minute=minuteOfDay%60;let phase='day',label='DAY',icon='☀';if(minuteOfDay<300||minuteOfDay>=1140){phase='night';label='NIGHT';icon='☾'}else if(minuteOfDay<420){phase='dawn';label='DAWN';icon='◐'}else if(minuteOfDay>=1020){phase='dusk';label='DUSK';icon='◑'}return{minuteOfDay,day,hour,minute,phase,label,icon,angle:minuteOfDay/1440*360}
}
function persistWorldClockV149(w=worldStateV14()){if(!profile||!w?.clockV149)return;const s=worldClockStateV149(w);const saved=ensureWorldProfileV14(profile);saved.clockMinutesV149=s.minuteOfDay;saved.worldDayV149=s.day}
function renderWorldClockV149(){const el=$('worldClockV149');if(!el||!profile)return;const s=worldClockStateV149();el.dataset.phase=s.phase;el.style.setProperty('--clock-angle',`${s.angle}deg`);$('worldClockIconV149').textContent=s.icon;$('worldClockTimeV149').textContent=`${String(s.hour).padStart(2,'0')}:${String(s.minute).padStart(2,'0')}`;$('worldClockPhaseV149').textContent=`${s.label} • D${s.day}`;el.setAttribute('aria-label',`World time ${s.hour}:${String(s.minute).padStart(2,'0')}, ${s.label.toLowerCase()}, day ${s.day}`)}
function drawWorldLightingV149(w=worldStateV14()){if(!w)return;const s=worldClockStateV149(w);let color='rgba(0,0,0,0)';if(s.phase==='night')color='rgba(7,17,37,.42)';else if(s.phase==='dawn')color='rgba(84,49,52,.14)';else if(s.phase==='dusk')color='rgba(73,34,44,.20)';if(color==='rgba(0,0,0,0)')return;ctx.save();ctx.fillStyle=color;ctx.fillRect(0,0,canvas.width,canvas.height);ctx.restore()}
...
function buildWorldStateV14(region='emberwood'){
  ...
  const gearDrops=[{id:'fieldblade1',x:10,y:9,name:'Field-Tested Weapon',type:'weapon',rarity:'uncommon',index:0,done:false},{id:'fieldmail1',x:18,y:6,name:'Field-Tested Chestguard',type:'chest',rarity:'uncommon',index:1,done:false},{id:'fieldring1',x:15,y:13,name:'Field-Tested Ring',type:'ring',rarity:'rare',index:2,done:false}];
  const entrances=[{id:'embercellar',x:27,y:9,name:'Emberroot Cellar',delveId:'embercellar'}];
  const saved=profile?ensureWorldProfileV14(profile):null,now=Date.now();return{active:true,region,map:worldMapV14(17),encounters,resources,gearDrops,entrances,startedAt:now,clockV149:{baseMinute:saved?.clockMinutesV149??420,day:saved?.worldDayV149??1,epochMs:now,minutesPerRealMinute:V149_WORLD_MINUTES_PER_REAL_MINUTE}}
}
...
function returnPartyToEmberwatchV142(reason='Returned to the Emberwatch bonfire.',opts={}){
  if(!isHost||!room||room.run)return false;persistWorldClockV149(room.worldV14);if(profile)persistProfile();resetToEmberwatchV142();hide('lobbyOverlay');if(opts.broadcast!==false)broadcastSnapshot();if(opts.render!==false){renderLobby();renderAll()}if(opts.notice!==false){flowBannerV13('EMBERWATCH',reason,'route','BONFIRE RETURN');notifyV11('info','EMBERWATCH BONFIRE',reason)}return true
}
...
function worldObjectsV141(w=worldStateV14()){if(!w)return[];return[...(w.encounters||[]).filter(worldEncounterAvailableV14).map(x=>({...x,objType:'encounter'})),...(w.resources||[]).filter(x=>!x.done).map(x=>({...x,objType:'resource'})),...(w.gearDrops||[]).filter(x=>!x.done).map(x=>({...x,objType:'gear'})),...(w.entrances||[]).map(x=>({...x,objType:'entrance'})),{id:'campgate',x:2,y:9,name:'Emberwatch Gate',objType:'gate'}]}
...
function worldGearRewardV149(obj,pid=peerId){const receiptId=`worldgear:${room?.worldV14?.startedAt||0}:${obj.id}:${pid}`,type=gearVisualTypeV149(obj.type),hunterLevel=room?.players?.[pid]?.level||profile?.level||1,ilvl=Math.max(2,Math.min(12,hunterLevel+2+(obj.index||0))),rarity=obj.rarity||'uncommon',mult=rarity==='rare'?1.38:1.16,item={id:`${receiptId}:item`,type,ilvl,rarity,name:obj.name||`Field-Tested ${slotDisplayTypeV132({type})}`,atk:0,def:0,hp:0,affixType:['atk','def','crit'][obj.index%3],affixValue:[2,2,.03][obj.index%3],enhance:0,zoneId:'frontier',worldPickupV149:true,value:ilvl*12};fillItemStatsV132(item,mult);return{receiptId,kind:'gear',gear:item,title:'EQUIPMENT RECOVERED',detail:`${item.name} • ${slotDisplayTypeV132(item)} • iLvl ${item.ilvl}`}}
function applyWorldResourceRewardV141(reward){if(!profile||!reward||profileReceiptSeenV145('worldRewardReceiptsV145',reward.receiptId))return false;const profileBeforeReward=structuredCloneSafe(profile);ensureWorldProfileV14(profile);if(reward.kind!=='gear')profile.worldV14.contracts.resources++;profile.gold+=(reward.gold||0);profile.materials.common+=(reward.common||0);profile.materials.rare+=(reward.rare||0);if(reward.gear)profile.inventory.push(normalizeItemV132(reward.gear));rememberProfileReceiptV145('worldRewardReceiptsV145',reward.receiptId);checkWorldContractsV14();if(!persistProfile()){profile=profileBeforeReward;renderAll();return false}notifyV11('loot',reward.title,reward.detail,{life:5600});renderAll();return true}
...
function hostWorldInteractV141(pid,objectId,autoAttack=false){
  ...
  if(o.objType==='gear'){const real=(room.worldV14.gearDrops||[]).find(x=>x.id===o.id);if(!real||real.done)return;const reward=worldGearRewardV149(real,pid);if(!deliverWorldRewardV141(pid,reward))return;real.done=true;broadcastSnapshot();return}
...
function drawWorldV14(){
  ...
  (w.gearDrops||[]).filter(o=>!o.done).forEach(o=>{const x=o.x*TILE+16,y=o.y*TILE+16,pulse=.82+Math.sin(performance.now()/210+(o.index||0))*.18;ctx.save();ctx.globalAlpha=pulse;ctx.fillStyle=o.rarity==='rare'?'rgba(74,139,223,.14)':'rgba(87,183,118,.12)';ctx.beginPath();ctx.arc(x,y,15,0,Math.PI*2);ctx.fill();drawGearObjectV149(o.type,x,y,o.rarity||'uncommon',.72);ctx.font='bold 6px monospace';ctx.fillStyle=o.rarity==='rare'?'#9ac8ff':'#9ce1ad';ctx.fillText(slotDisplayTypeV132(o).toUpperCase(),x,y-14);ctx.restore()});
...
let huntMapOpenV150=false;
function deepHuntAvailableV150(){return !!(room||snapshot)?.run?.deepHunt?.active}
function setHuntMapV150(open){
  const overlay=$('huntMapOverlayV150'),canOpen=deepHuntAvailableV150(),next=!!open&&canOpen;if(!overlay)return false;
  huntMapOpenV150=next;overlay.classList.toggle('show',next);overlay.setAttribute('aria-hidden',String(!next));document.body.classList.toggle('hunt-map-open-v150',next);[$('huntMapBtnV150'),$('deepSummaryMapBtnV150')].forEach(b=>b?.setAttribute('aria-pressed',String(next)));if(next){HUD_RIGHT_GROUP_V141.forEach(id=>setHudPanelV141(id,false));renderDeepHuntPanel((room||snapshot)?.run);setTimeout(()=>$('huntMapCloseV150')?.focus(),0)}else $('huntMapBtnV150')?.focus();return next
}
function toggleHuntMapV150(){return setHuntMapV150(!huntMapOpenV150)}
function renderDeepHuntPanel(run){
  const panel=$('expeditionPanel'),summary=$('deepSummaryV150'),mapButton=$('huntMapBtnV150'); if(!panel) return;
  if(!run?.deepHunt?.active){panel.classList.remove('show');panel.style.display='none';panel.innerHTML='<div class="hunt-map-empty-v150">A Deep Hunt route will appear here after launch.</div>';if(summary)summary.hidden=true;if(mapButton){mapButton.disabled=true;mapButton.setAttribute('aria-disabled','true')}if(huntMapOpenV150)setHuntMapV150(false);return}
  ...
  if(summary){summary.hidden=false;$('deepSummaryDepthV150').textContent=`${run.depth}/${runDepthLimit(run)}`;$('deepSummaryHeatV150').textContent=`+${Math.round((huntHeatV10(run)-1)*100)}%`;$('deepSummarySpoorV150').textContent=`${deep.spoor}/${deep.cfg.spoorRequired}`;const extract=!!cur?.canExtract;$('deepSummaryExtractV150').textContent=extract?'READY':'LOCKED';summary.querySelector('.deep-extract-v150')?.classList.toggle('is-ready',extract)}if(mapButton){mapButton.disabled=false;mapButton.removeAttribute('aria-disabled')}
...
function renderInventoryV131(inv){if(!inv)return;inv.innerHTML='';const list=filteredInventoryV131();if(!list.length){inv.innerHTML='<div class="tiny">No equipment matches these filters.</div>';return}list.forEach(i=>{normalizeItemV132(i);const equipped=comparisonItemForV132(profile,i),slot=resolveEquipSlotV132(i,profile),cmp=gearScore(i)-gearScore(equipped),cls=cmp>1?'compareup':cmp<-1?'comparedown':'compareflat',d=document.createElement('div');d.className=`invrow gear-row-v149 ${i.favoriteV131?'v131-favorite':''} ${i.lockedV131?'v131-locked':''}`;d.innerHTML=`${gearSpriteHTMLV149(i)}<div><span class="rarity-${i.rarity}">${i.favoriteV131?'★ ':''}${i.lockedV131?'🔒 ':''}${i.name}</span><div class="tiny">${slotDisplayTypeV132(i)} • iLvl ${i.ilvl} • ${itemStats(i)} • <span class="${cls}">${cmp>1?'▲ upgrade':cmp<-1?'▼ weaker':'≈ sidegrade'}</span></div>${itemEffectTagsHTMLV132(i)}</div><div class="inv-actions-v131"><button onclick="equipRecovered('${i.id}')">Equip${slot?` → ${ARMORY_SLOT_LABEL_V132[slot]}`:''}</button><button onclick="toggleFavoriteV131('${i.id}')">${i.favoriteV131?'★':'☆'}</button><button onclick="toggleLockV131('${i.id}')">${i.lockedV131?'Unlock':'Lock'}</button><button ${i.lockedV131?'disabled':''} onclick="salvageItem('${i.id}')">Salvage</button></div>`;inv.appendChild(d)})}
...
function onNetwork(msg){
  ...
  if(isHost){
    ...
    if(msg.type==='leave'&&msg.peerId===room?.hostPeerId){if(room?.run||room?.worldV14)closeRemoteRoomV147('The host left during an active hunt, so the field session was safely closed.');else notifyV11('warn','HOST LEFT THE PARTY','Waiting for Emberwatch leadership handoff.',{life:7000});return}
...
function renderMinimap(r){
  ...
  if(run?.map){
    ...
    const me=run.players?.[peerId],boss=run.enemies?.find(e=>e.boss&&e.hp>0),remaining=run.enemies.filter(e=>e.hp>0).length,challenge=run.deepChallenge&&!run.deepChallenge._resolved?run.deepChallenge:null;$('objectiveLines').innerHTML=`<b>${MISSIONS[run.missionId].name}</b> · ${run.zoneStageName||zoneStageName(run)}<br>${boss?`Break ${boss.name} · Phase ${boss.bossPhase||1}`:`Clear the area · ${remaining} enem${remaining===1?'y':'ies'} remain`}<br>${challenge?`<span class="bosswarn">${challenge.label}${challenge.deadline?` · before R${challenge.deadline}`:''}</span>`:`Round ${run.round} · ${run.phase.toUpperCase()} · Carry ${carriedLootValueV10(me).toLocaleString()}`}`;
  }else if(world?.map){
...
function renderCombatVitalsV149(r){
  const el=$('combatVitalsV149'),status=$('combatStatusEffectsV150');if(!el)return;if(!profile){el.style.display='none';if(status)status.innerHTML='';return}el.style.display='grid';const run=r?.run,c=CLASSES[profile.classId],active=run?.players?.[peerId],base=active||trainingPlayer(),hp=active?Math.max(0,active.hp):base.maxHp,maxHp=Math.max(1,base.maxHp||1),res=active?Math.max(0,active.res):base.maxRes,maxRes=Math.max(1,base.maxRes||c.maxRes||1),state=active?(active.dead?'FALLEN':active.downed?'DOWNED':run.bossTelegraph?'TELEGRAPH':run.phase==='player'?'HUNTING':run.phase.toUpperCase()):worldStateV14()?'SURFACE':'EMBERWATCH';$('combatVitalsNameV149').textContent=`${profile.name} · LV ${profile.level}`;$('combatVitalsStateV149').textContent=state;$('combatVitalsGearV149').textContent=c.name.toUpperCase();$('combatResLabelV149').textContent=c.res.toUpperCase().slice(0,5);$('combatHpFillV149').style.width=`${clamp(hp/maxHp*100,0,100)}%`;$('combatResFillV149').style.width=`${clamp(res/maxRes*100,0,100)}%`;$('combatHpTextV149').textContent=active?`${hp}/${maxHp}`:'READY';$('combatResTextV149').textContent=active?`${res}/${maxRes}`:'READY';if(status){const tags=[];if(active?.guarding)tags.push('<span class="good">GUARDING</span>');if(active?.frenzyCharges>0)tags.push(`<span class="good">FRENZY ×${active.frenzyCharges}</span>`);if(active?.status?.poison)tags.push(`<span class="bad">POISON ${active.status.poison}</span>`);if(active?.status?.burn)tags.push(`<span class="bad">BURN ${active.status.burn}</span>`);if(active?.status?.bleed)tags.push(`<span class="bad">BLEED ${active.status.bleed}</span>`);if(active?.classId==='berserker')tags.push('<span class="quick">QUICK ACTION — DOES NOT END TURN</span>');status.innerHTML=tags.slice(0,4).join('')}}
function renderBossCombatHudV150(run){
  const hud=$('bossCombatHudV150');if(!hud)return;const boss=run?.enemies?.find(e=>e.boss&&e.hp>0);if(!boss){hud.classList.remove('show');return}hud.classList.add('show');const hp=Math.max(0,boss.hp),max=Math.max(1,boss.maxHp),parts=(boss.parts||[]).map(p=>`${p.name} ${p.broken?'BROKEN':p.id==='heart'&&!boss.heartExposed?'SEALED':Math.max(0,p.hp)}`).join(' · ')||'NO BREAKABLE PARTS',telegraph=run.bossTelegraph||boss.intentLabel||boss.intent?.replaceAll('_',' ')||'TRACKING';$('bossCombatNameV150').textContent=boss.name.toUpperCase();$('bossCombatPhaseV150').textContent=`PHASE ${boss.bossPhase||1}${boss.kind==='worldeater'?' / 4':''}`;$('bossCombatHpV150').textContent=`${hp.toLocaleString()} / ${max.toLocaleString()}`;$('bossCombatFillV150').style.width=`${clamp(hp/max*100,0,100)}%`;$('bossCombatPartsV150').textContent=parts;$('bossCombatTelegraphV150').textContent=telegraph.toUpperCase();$('bossCombatTelegraphV150').classList.toggle('is-telegraph',!!(run.bossTelegraph||boss.intent))
}
...
function renderAll(){
  const r=room||snapshot;updateHudModeV141(r);draw(r?.run);renderParty(r);renderPlayer(r);renderRunLoot(r);renderLog(r);renderActions(r);renderCombatMeters(r);renderMinimap(r);renderGrandHuntHud(r?.run);renderCombatHudV11(r);renderCombatVitalsV149(r);renderBossCombatHudV150(r?.run);renderWorldClockV149();syncHudSignalsV13(r);const wb=$('worldRegionBannerV14');if(wb)wb.classList.toggle('show',!!worldStateV14()&&!r?.run);renderLobby()
}
...
function renderActions(r){
  ...
  const can=!!run&&run.phase==='player'&&me&&!me.dead&&!me.downed&&!me.submitted&&!remoteTransportV141.recoveryRequired;
  const target=run?.enemies?.find(e=>e.id===selectedTargetId&&e.hp>0);if(selectedTargetId&&!target)selectedTargetId=null;
  const commandBar=document.querySelector('.command-bar-v148');
  let commandState='idle';
  if(run){
    if(remoteTransportV141.recoveryRequired)commandState='frozen';
    else if(run.phase==='choice')commandState='choice';
    else if(run.phase==='path')commandState='path';
    else if(run.phase==='settlement_failed')commandState='settlement';
    else if(me?.dead||me?.downed)commandState='incapacitated';
    else if(me?.submitted)commandState='submitted';
    else if(run.phase==='player')commandState='active';
    else commandState='resolving';
  }
  if(commandBar){commandBar.dataset.commandState=commandState;commandBar.classList.toggle('has-target-v148',!!target);commandBar.classList.toggle('has-parts-v148',!!target?.parts?.some(part=>partTargetable(target,part)));commandBar.classList.toggle('boss-target-v148',!!target?.boss)}
  ...
  $('reviveBtn').disabled=!can||!findAdjacentDownedClient(run,me);if(can&&me&&((me.potions||0)<=0||me.hp>=me.maxHp))$('potionBtn').disabled=true;const cancelB=$('cancelActionV131');if(cancelB){cancelB.style.display=run?.phase==='player'&&me?.submitted?'block':'none';cancelB.disabled=!!remoteTransportV141.recoveryRequired}
  ...
  $('votePanel').style.display=run?.phase==='choice'?'grid':'none';$('extractBtn').disabled=!run||!!run.votes?.[peerId]||!!remoteTransportV141.recoveryRequired;$('descendBtn').disabled=!run||!!run.votes?.[peerId]||!!remoteTransportV141.recoveryRequired;if(run){const carried=carriedLootValueV10(me),safe=deepCurrentNode(run)?.canExtract; $('extractBtn').textContent=safe?`Extract Here — Secure ~${carried.toLocaleString()} value`:`Field Extract — Save ~${Math.round(carried*.85).toLocaleString()} value`; $('descendBtn').textContent=run.deepHunt?.active?`Choose Route — Heat +12%`:`Push Deeper — Heat +12%`;}
  ...
  else if(remoteTransportV141.recoveryRequired)$('actionHint').textContent='Host reconnect required. The active field is frozen and no commands can be submitted.';
  ...
  $('actionHint').dataset.tone=remoteTransportV141.recoveryRequired||run?.bossTelegraph?'danger':me?.submitted||run?.phase==='resolving'?'waiting':run?.phase==='choice'||run?.phase==='path'?'decision':'normal';
  ...
  const targetInfo=$('targetInfo'),targetDistance=target&&me?distanceToEnemy(me,target):null,selectedPartV150=target?.parts?.find(part=>part.id===selectedPartId&&partTargetable(target,part))||target?.parts?.find(part=>partTargetable(target,part))||null;targetInfo.dataset.commandState=commandState;targetInfo.classList.toggle('is-boss-v150',!!target?.boss);
  if(target){
    const hpPct=clamp(Math.round(Math.max(0,target.hp)/Math.max(1,target.maxHp)*100),0,100),alerted=target.alerted||enemyForcedAggroV132(run,target),intentLabel=target.intentLabel||target.intent?.replaceAll('_',' ').toUpperCase();
    targetInfo.setAttribute('aria-label',`${target.name}, ${Math.max(0,target.hp)} of ${target.maxHp} health${targetDistance!=null?`, ${targetDistance} tiles away`:''}. Hover or focus for combat details.`);
    targetInfo.innerHTML=`<div class="target-ribbon-v148"><span class="target-lock-v148 ${target.boss?'boss':alerted?'alert':''}">${target.boss?'BOSS':target.elite||/ELITE|APEX|LIEUTENANT/.test(target.trait||'')?'ELITE':alerted?'ALERT':'LOCKED'}</span><b class="target-name-v148 ${target.boss?'gold':''}">${target.name}</b>${selectedPartV150?`<span class="target-part-v150">AIM ${selectedPartV150.name.toUpperCase()}</span>`:''}${targetDistance!=null?`<span class="target-range-v148">${targetDistance} TILE${targetDistance===1?'':'S'}</span>`:''}<span class="target-hp-meter-v148"><i style="width:${hpPct}%"></i></span><span class="target-hp-v148">${Math.max(0,target.hp)}/${target.maxHp}</span>${intentLabel?`<span class="target-intent-v148">⚠ ${intentLabel}</span>`:''}</div><div class="target-detail-card-v148"><div><b>${target.boss?'BOSS KIT':'ABILITY'}:</b> ${enemyAbilityTextV09(target)}</div><div class="target-detail-stats-v148">DEF ${target.def}${target.boss?` • COLOSSAL ${target.size}×${target.size} • PHASE ${target.bossPhase||1} • ${target.partySize}P SCALE`:''}${target.trait?` • ${target.trait}`:''}${!target.boss?` • ${alerted?'ALERTED':`DORMANT • AGGRO ${enemyAggroRangeV132(target)}`}`:''}</div>${target.boss&&bossCounterHintV131(target)?`<div class="counter-hint-v131"><b>COUNTERPLAY:</b> ${bossCounterHintV131(target)}</div>`:''}${target.intent?`<div class="bosswarn">⚠ <span class="boss-ability">${intentLabel}</span> CHARGING</div>`:''}${run.stageEvent?`<div class="eventwarn">${run.stageEvent.name}: ${run.stageEvent.desc}</div>`:''}<div class="target-shortcuts-v131"><kbd>Tab</kbd> cycle enemy • <kbd>Q</kbd> cycle body part</div></div>`;
  }else{
    targetInfo.setAttribute('aria-label','Automatic nearest-enemy targeting. Click an enemy to lock it.');
    targetInfo.innerHTML='<div class="target-ribbon-v148"><span class="target-lock-v148">AUTO</span><b class="target-name-v148">Nearest enemy</b><span class="target-auto-v148">Click a target to lock</span><span class="target-more-v148">DETAILS</span></div><div class="target-detail-card-v148">Basic attacks and single-target skills choose the nearest valid enemy until you lock one.<div class="target-shortcuts-v131"><kbd>Tab</kbd> cycle enemy • <kbd>Q</kbd> cycle body part</div></div>';
  }
  ...
  const pathPanel=$('pathPanel');pathPanel.style.display=run?.phase==='path'?'grid':'none';pathPanel.innerHTML='';if(run?.phase==='path'){(run.pathChoices||[]).slice(0,3).forEach((path,i)=>{const b=document.createElement('button'),danger=Math.round(((path.enemy||1)-1)*100),reward=Math.round(((path.reward||1)-1)*100);b.className='pathcard';b.disabled=!isHost;b.innerHTML=`<b>${deepNodeIcon(path.type||'hunt')} ${path.name}</b><span>${path.desc}</span><div class="tagline"><span class="expchip">${deepNodeTypeName(path.type||'hunt')}</span><span class="expchip heat">${danger>0?`DANGER +${danger}%`:'STANDARD DANGER'}</span>${path.track?`<span class="expchip spoor">+${path.track} SPOOR</span>`:''}${path.canExtract?'<span class="expchip extract">EXTRACTION</span>':''}${path.boon?`<span class="expchip">BOON ${boonById(path.boon)?.name||path.boon}</span>`:reward?`<span class="expchip">REWARD +${reward}%</span>`:''}</div>`;b.onclick=()=>choosePathChoice(i);pathPanel.appendChild(b)})}renderDeepHuntPanel(run);
  actionPolishV13(run,me,c);
  [["attackBtn","Attack","1"],["skill1Btn",c?.s1.name||"Skill one","2"],["skill2Btn",c?.s2.name||"Skill two","3"],["guardBtn","Guard","4"],["potionBtn",`Potion, ${me?.potions??3} remaining`,"5"],["reviveBtn","Revive adjacent ally","6"]].forEach(([id,label,key])=>{const button=$(id);if(!button)return;button.setAttribute('aria-label',`${label}. Shortcut ${key}.`);button.setAttribute('aria-keyshortcuts',key)});
...
function draw(run){
  ...
  drawWorldFx();
  drawCombatFloaters();
  if(run.isWorldSkirmish)drawWorldLightingV149(worldStateV14());
  // Boss status is mounted in the edge HUD; keep the battlefield canvas free of duplicate bars.
...
function armorySlotHTMLV132(slot){
  const i=profile?.equipment?.[slot],label=ARMORY_SLOT_LABEL_V132[slot];
  return `<div class="armory-slot-v132 ${i?'':'empty'}" data-armory-slot="${slot}">${i?gearSpriteHTMLV149(i,{label:`${label} equipment`}):gearSpriteHTMLV149(slotFamilyV132(slot),{label:`Empty ${label} slot`})}<div><span class="slot-label">${label}</span><span class="slot-name ${i?`rarity-${i.rarity}`:''}">${i?i.name:'Empty'}</span><div class="slot-mini">${i?`iLvl ${i.ilvl} • GR ${itemProgressionRating(i)}${i.enhance?` • +${i.enhance}`:''}`:'Click to browse compatible gear'}</div>${i?itemEffectTagsHTMLV132(i):''}</div></div>`
}
...
function setHudPanelV141(id,force){
  ...
  if(next&&huntMapOpenV150)setHuntMapV150(false);
...
const HUD_CONTEXT_V148={mode:null};
function syncHudContextV148(r,activeHunter){
  const mode=!activeHunter?'none':r?.run?'run':r?.worldV14?'world':'camp';if(HUD_CONTEXT_V148.mode===mode)return;HUD_CONTEXT_V148.mode=mode;document.body.dataset.hudContext=mode;
  if(mode==='run'){setHudPanelV141('party',false);setHudPanelV141('intel',false)}
  else if(mode==='world'){setHudPanelV141('party',false);setHudPanelV141('intel',false)}
  else if(mode==='camp'){setHudPanelV141('party',false);setHudPanelV141('intel',false)}
}
function updateHudModeV141(r){const activeHunter=!!profile&&!!r?.players?.[peerId];document.body.classList.toggle('has-profile-v141',activeHunter);document.body.classList.toggle('mode-run-v141',!!r?.run);document.body.classList.toggle('mode-world-v141',!!r?.worldV14&&!r?.run);document.body.classList.toggle('mode-camp-v141',activeHunter&&!r?.run&&!r?.worldV14);syncHudContextV148(r,activeHunter)}
function initHudV141(){document.querySelectorAll('[data-hud-toggle]').forEach(button=>button.addEventListener('click',()=>setHudPanelV141(button.dataset.hudToggle)));setHudPanelV141('party',false);setHudPanelV141('intel',false);setHudPanelV141('meters',false);setHudPanelV141('loot',false);setHudPanelV141('log',false)}
...
async function leaveRemoteSessionV147(state,activeField=false){
  ...
  if(!state.authorized){stopRemoteTransportV141();return}state.departing=true;state.controller?.abort();let pending=null;if(state.snapshotTimer){clearTimeout(state.snapshotTimer);state.snapshotTimer=null;pending=state.pendingSnapshot;state.pendingSnapshot=null}try{if(pending)await remotePostV141(pending,state.room);else await state.postQueue.catch(()=>false);await remotePostV141({type:'leave',peerId},state.room).catch(()=>false);await fetch('/api/multiplayer',{method:'POST',headers:remoteHeadersV147(state),credentials:'same-origin',body:JSON.stringify({op:'leave',room:state.room,peerId,activeField:!!activeField})}).catch(()=>null)}finally{clearRemoteSessionV147();if(remoteTransportV141===state)stopRemoteTransportV141()}
}
function resetRoomUiV147(silent=false){room=null;snapshot=null;v7ClassMotion.clear();clearHeldMovement();combatFloaters=[];combatFeed=[];worldFx=[];isHost=false;roomCode=null;$('roomChoice').style.display='block';$('roomPanel').style.display='none';$('roomBadge').textContent='No room';hide('lobbyOverlay');hide('campServiceOverlay');hide('trainingOverlay');hide('wagerOverlay');if(!silent)show('titleOverlay');updateSaveTransferUiV144();renderAll()}
function closeRemoteRoomV147(message='The multiplayer room closed'){
  const state=remoteTransportV141;if(state.departing)return;state.departing=true;clearRemoteSessionV147();stopRemoteTransportV141();resetRoomUiV147(false);notifyV11('warn','PARTY SESSION CLOSED',message,{life:7000})
}
function leaveRoom(silent=false){
  const state=remoteTransportV141,activeField=!!(room?.run||room?.worldV14);clearRemoteSessionV147();leaveRemoteSessionV147(state,activeField);resetRoomUiV147(silent)
}
...
$('titleBtn').onclick=()=>leaveRoom(false);$('soundBtn').onclick=()=>{soundOn=!soundOn;const b=$('soundBtn'),label=`Sound ${soundOn?'on':'off'}`;b.innerHTML=`<span aria-hidden="true">${soundOn?'♪':'×'}</span><span class="sr-only-v150">${label}</span>`;b.title=label;b.setAttribute('aria-label',label);b.setAttribute('aria-pressed',String(soundOn));if(soundOn)beep(440,.03,'sine')};
$('huntMapBtnV150').onclick=toggleHuntMapV150;$('deepSummaryMapBtnV150').onclick=toggleHuntMapV150;$('huntMapCloseV150').onclick=()=>setHuntMapV150(false);$('objectiveToggleV150').onclick=()=>{const tracker=$('objectiveTrackerV150'),collapsed=tracker.classList.toggle('is-collapsed'),button=$('objectiveToggleV150');button.setAttribute('aria-expanded',String(!collapsed));button.title=collapsed?'Expand objectives':'Collapse objectives';button.querySelector('i').textContent=collapsed?'+':'−'};
...
window.addEventListener('keydown',e=>{
  ...
  if(huntMapOpenV150&&(e.key==='Escape'||key==='m')){e.preventDefault();setHuntMapV150(false);return}
  ...
  if(run?.deepHunt?.active&&key==='m'){e.preventDefault();toggleHuntMapV150();return}
...
function spriteAnimationLoop(ts){
  ...
  if(ts-lastMeterPaint>=180){renderCombatMeters(room||snapshot);renderMinimap(room||snapshot);renderWorldClockV149();renderCombatVitalsV149(room||snapshot);renderBossCombatHudV150((room||snapshot)?.run);lastMeterPaint=ts}
