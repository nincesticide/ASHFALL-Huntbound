...
1. Complete or safely extract from Emberroot Cellar with two clients and verify each character-bound settlement once.
2. Force a multiplayer wipe and verify both clients return to the Emberwatch bonfire.
3. Drop the host during an active field state and verify the room freezes or closes without hidden authority migration.
